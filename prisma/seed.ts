...
import { ensureTenantHierarchyRoles } from "../src/services/tenantBootstrap.service.js";

const prisma = new PrismaClient();

const PERMISSIONS: { action: string; module: string }[] = [
...
async function main() {
  ...
  const seedDemo = process.env.SEED_DEMO_TENANT === "true";
  if (!seedDemo) {
    console.log("Seed: demo tenant skipped (set SEED_DEMO_TENANT=true to enable)");
    return;
  }
  ...
  await ensureTenantHierarchyRoles(demoTenant.id);
  const adminR = await prisma.role.findFirstOrThrow({
    where: { tenantId: demoTenant.id, code: "ADMIN" },
  });
  const mgrR = await prisma.role.findFirstOrThrow({
    where: { tenantId: demoTenant.id, code: "MANAGER" },
  });
  const staffR = await prisma.role.findFirstOrThrow({
    where: { tenantId: demoTenant.id, code: "STAFF" },
  });
  ...
  console.log(
    "Seed demo users: admin@demo.co, manager@demo.co, staff@demo.co /",
    tenantPass,
  );
}
