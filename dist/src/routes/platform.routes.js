...
router.get("/tenants", requirePermission("platform.tenant.list"), async (req, res) => {
    const q = z
        .object({
        page: z.preprocess((v) => (v === undefined || v === "" ? 1 : v), z.coerce.number().int().min(1)),
        pageSize: z.preprocess((v) => (v === undefined || v === "" ? 10 : v), z.coerce.number().int().min(1).max(200)),
        search: z
            .string()
            .optional()
...
router.get("/dashboard", requirePermission("platform.tenant.list"), async (_req, res) => {
...
router.post("/tenants", requirePermission("platform.tenant.create"), async (req, res) => {
    const body = z
        ...
        .parse(req.body);
    try {
        const { tenant, inviteLink } = await platformService.createTenantWithInvitation(body);
...
router.patch("/tenants/:id/status", requirePermission("platform.tenant.manage"), async (req, res) => {
...
router.post("/tenants/:id/reinvite", requirePermission("platform.tenant.manage"), async (req, res) => {
    const params = z.object({ id: z.string().min(1) }).parse(req.params);
...
router.delete("/tenants/:id", requirePermission("platform.tenant.manage"), async (req, res) => {
    const params = z.object({ id: z.string().min(1) }).parse(req.params);
    try {
