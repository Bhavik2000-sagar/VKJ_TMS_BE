...
import { ensureTenantHierarchyRoles, TENANT_ASSIGNABLE_ROLE_CODES, } from "../services/tenantBootstrap.service.js";
const router = Router();
...
router.patch("/users/:id", requirePermission("user.manage"), async (req, res) => {
    const params = z.object({ id: z.string().min(1) }).parse(req.params);
    const body = z
        .object({
...
router.patch("/users/:id/status", requirePermission("user.manage"), async (req, res) => {
    const params = z.object({ id: z.string().min(1) }).parse(req.params);
