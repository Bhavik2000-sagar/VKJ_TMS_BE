...
async function canViewTask(userId, tenantId, task) {
    ...
    if (full.assignedToId && scope.has(full.assignedToId))
        return true;
    if (full.createdById && scope.has(full.createdById))
        return true;
    return false;
}
async function taskVisibilityOrClause(userId, tenantId) {
    const perms = await getEffectivePermissionActions(userId);
    const subs = await getSubordinateIds(userId);
...
function taskListOrderBy(sortBy, sortDir) {
    const d = sortDir;
    switch (sortBy) {
        case "title":
            return { title: d };
...
function queueFilter(userId, queue) {
    if (!queue || queue === "all")
        return null;
    switch (queue) {
        case "my":
...
export async function listTasksPaginated(userId, tenantId, params) {
    const or = await taskVisibilityOrClause(userId, tenantId);
    const andFilters = [];
    ...
    if (qf)
        andFilters.push(qf);
    if (params.statusId)
        andFilters.push({ statusId: params.statusId });
    if (params.priority)
        andFilters.push({ priority: params.priority });
    if (params.dueFrom)
        andFilters.push({ dueDate: { gte: params.dueFrom } });
    if (params.dueTo)
        andFilters.push({ dueDate: { lte: params.dueTo } });
...
export async function getTask(userId, tenantId, taskId) {
...
export async function createTask(userId, tenantId, data) {
...
export async function updateTask(userId, tenantId, taskId, data) {
    ...
    if (existing.assignedToId)
        notifyIds.add(existing.assignedToId);
    if (data.assignedToId && data.assignedToId !== existing.assignedToId) {
        notifyIds.add(data.assignedToId);
    }
    if (existing.reviewerId)
        notifyIds.add(existing.reviewerId);
...
export async function reviewTask(userId, tenantId, taskId, decision, comment) {
    const task = await prisma.task.findFirst({
        where: { id: taskId, tenantId },
        include: { status: true },
    });
    if (!task || task.reviewerId !== userId)
        return null;
    ...
    if (decision === "approve" && done)
        nextStatusId = done.id;
    else if (decision === "reject" && wip)
        nextStatusId = wip.id;
    ...
    const baseMsg = decision === "approve" ? "Review approved" : "Review rejected — sent back";
...
async function resolveStatusIdByCode(tenantId, code) {
    const s = await prisma.taskStatus.findFirst({ where: { tenantId, code } });
    return s?.id ?? null;
}
export async function acceptTask(userId, tenantId, taskId) {
    const existing = await prisma.task.findFirst({
        where: { id: taskId, tenantId },
        include: { status: true },
    });
    ...
    const nextStatusId = existing.status.code === "TODO"
...
export async function startTask(userId, tenantId, taskId) {
    const existing = await prisma.task.findFirst({
        where: { id: taskId, tenantId },
        include: { status: true },
    });
...
export async function addTimeLog(userId, tenantId, taskId, minutes) {
    const existing = await prisma.task.findFirst({
        where: { id: taskId, tenantId },
    });
    if (!existing)
...
export async function addComment(userId, tenantId, taskId, message) {
    const existing = await prisma.task.findFirst({
        where: { id: taskId, tenantId },
    });
    if (!existing)
...
export async function addAttachment(userId, tenantId, taskId, fileUrl, fileName, mimeType, checklistItemId) {
    const existing = await prisma.task.findFirst({
        where: { id: taskId, tenantId },
    });
    if (!existing)
...
export async function applyTemplateChecklistToTask(userId, tenantId, taskId, templateId) {
...
export async function listTaskChecklistItems(userId, tenantId, taskId) {
    const task = await prisma.task.findFirst({ where: { id: taskId, tenantId } });
    if (!task)
        return null;
    const ok = await canViewTask(userId, tenantId, task);
...
export async function updateTaskChecklistItem(userId, tenantId, taskId, itemId, data) {
    const task = await prisma.task.findFirst({ where: { id: taskId, tenantId } });
    if (!task)
        return null;
    const ok = await canViewTask(userId, tenantId, task);
    ...
    const checkedAt = data.isChecked === undefined
        ? existing.checkedAt
        : isCheckedNext
            ? new Date()
            : null;
    const checkedById = data.isChecked === undefined
    ...
    await prisma.taskActivity.create({
        data: {
            ...
            userId,
            type: "STATUS_CHANGE",
            message: data.isChecked !== undefined
...
export async function deleteTask(userId, tenantId, taskId) {
    const existing = await prisma.task.findFirst({
        where: { id: taskId, tenantId },
    });
    if (!existing)
