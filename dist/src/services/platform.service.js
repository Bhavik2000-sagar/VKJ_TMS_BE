...
export async function listTenantsPaginated(input) {
    ...
    const where = q
        ...
        : {};
    const orderBy = input.sortBy === "users"
...
async function createAndSendTenantInvitation(input) {
    ...
    try {
        ...
        console.warn("[email] Failed to send tenant invitation email:", e?.message ?? e);
    }
    return inviteLink;
}
...
export async function reinviteTenantAdmin(input) {
    ...
    if (!last)
        throw new Error("No invitation email found for this tenant");
    const inviteLink = await createAndSendTenantInvitation({
        tenantId: tenant.id,
        tenantName: tenant.name,
        email: last.email,
